...
# Retry mechanics themselves (429 backoff, transient connection failures, give-up-after-max) are
# generic and tested once against `http_retry.fetch_with_retry` directly in test_http_retry.py.
# What's client-specific -- and still worth covering here -- is that the API key never leaks into
# a log line, even though the actual logging now happens down inside the shared retry helper.
