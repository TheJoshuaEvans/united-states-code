...
All three are fanned out from the same in-memory parsed node per citation in a single pass — one internal representation, three serializers — so they can't drift from each other. Directory layout: format-first, e.g. `usc/xml/26/501/c/3.xml`, `usc/json/26/501/c/3.json`, `usc/txt/26/501/c/3.txt`.
...
None currently — architecture decisions (hosting, formats, cadence, chunking granularity) are all settled. Next step is implementation.
