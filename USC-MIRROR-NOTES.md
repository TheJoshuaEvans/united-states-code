...
- **Appendix titles (5A/11A/18A/28A) don't get `.json` or `.txt` yet.** Their whole-file `full.xml` is structurally very different (court rules, reorganization plans, real `<table>` markup) from an ordinary per-section chunk; rendering them is deliberately deferred rather than folded into the first pass.
...
- **`.txt` rendering is implemented** (`render_txt.py`), reading `usc/*.xml` the same way `render_json.py` does. Unlike JSON, it's a *curated* rendering, not a structural mirror: `<note>`/`<notes>`/`<sourceCredit>` are dropped entirely (text and descendants), and a footnote-reference `<ref class="footnoteRef">` marker is dropped too (the only `class` value ever used on `<ref>` in the corpus) — but a dropped element's *tail* is kept, since the tail is the parent's running sentence continuing after it, not part of the excluded element. **Line-break rule, user decision 2026-07-12:** USLM stores no punctuation between a `<num>`/`<heading>` and the text that follows (e.g. `<num>(a)</num><heading> Exemption from taxation</heading><content>An organization...` — nothing between "taxation" and "An"), so naive `itertext()`-style concatenation runs words together. Chosen fix: every element is a "block" (starts a new line) *except* a small inline set (`ref`, `date`, `i`, `b`, `sup`, `sub`, `inline`, `quotedContent`) that's embedded mid-sentence in real usage and would fracture prose if forced onto its own line. Whitespace within a line is collapsed (any run, including a source line-wrap, becomes one space) and empty lines are dropped. This was picked over two alternatives: raw concatenation (matches JSON's "no interpretation" philosophy exactly but reads badly) and inventing official-print punctuation like `.—` (most readable, but a curated formatting rule rather than a mechanical structural join). Verified against the full corpus: all 50,936 non-appendix sections render without error (appendix `full.xml`s correctly skip via the same `NotASectionError` pattern as JSON).
